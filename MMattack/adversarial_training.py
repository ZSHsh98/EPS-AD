...
parser = argparse.ArgumentParser(description='PyTorch AT')
...
parser.add_argument('--out_dir', type=str, default='./CE_resnet18', help='dir of output')
...
def train(model, train_loader, optimizer):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        output_adv = attack.pgd(model,data,target,epsilon=args.epsilon,step_size=args.step_size,
                        num_steps=args.num_steps,loss_fn='cent',category="Madry",rand_init=args.rand_init)
        output_natural = data
...
if args.resume:
    ...
    logger_test.set_names(['Epoch', 'Natural Test Acc', 'train_loss', 'PGD20 Acc'])
...
for epoch in range(start_epoch, args.epochs):
    ...
    print(
        'Epoch: [%d | %d] | Train Time: %.4f s | Natural Test Acc %.4f | train_loss %.4f | PGD20 Test Acc %.4f |\n' % (
            epoch + 1,
            args.epochs,
            train_time,
            test_nat_acc,
    ...
    logger_test.append([epoch + 1, test_nat_acc, train_loss, test_pgd20_acc])
