...
if 1:
	...
	if dataset == 'imagenet':
		...
		# path_adv1 = f'score_diffusion_t_{dataset}_noperturb_mmd/scores_adv_FGSM_0.00392_5{t}{isperb_image}.npy'
		# path_adv2 = f'score_diffusion_t_{dataset}_noperturb_mmd/scores_adv_FGSM_L2_0.00392_5{t}{isperb_image}.npy'
		# perturb
		path_cln = f'/mnt/cephfs/ec/home/zhangshuhai/score_diffusion_t_cifar/scores_clean200{isperb_image}{data_size}.npy'
		path_adv1 = f'score_diffusion_t_{dataset}_stand/scores_adv_FGSM_0.00392_5200{isperb_image}.npy'
		path_adv2 = f'score_diffusion_t_{dataset}_stand/scores_adv_FGSM_L2_0.00392_5200{isperb_image}.npy'
				...
				for t in [tt]: #20, 50, 100, 150, 200]:
					...
					with torch.no_grad():
						...
						if dataset == 'cifar':
							...
							ref_data = torch.from_numpy(np.load(path_ref)).cuda()
							x_cln = torch.from_numpy(np.load(path_cln)[:100]).cuda()
							x_adv = torch.from_numpy(np.load(path_adv)[:100]).cuda()
						# print(x_cln.shape)
						# print(x_adv.shape)
