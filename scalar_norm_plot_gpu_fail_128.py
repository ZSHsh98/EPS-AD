...
# t = 100
attack_methods=['FGSM_L2', 'BIM_L2','BIM',  'MIM', 'TIM', 'DI_MIM',  'PGD_L2', 'FGSM', 'CW',]#['PGD','FGSM', 'CW', 'BIM',  'MIM', 'TIM', 'DI_MIM', 'FGSM_L2', 'BIM_L2', 'PGD_L2']
# attack_method = 'PGD'
...
for attack_method in attack_methods:
	print(f"======attack_method: {attack_method}")
	for t in ['noaverage']: #, 5, 10, 20, 50, 120, 150]
		with torch.no_grad():
			...
			log_dir = f'score_diffusion_t_{dataset}/'
			os.makedirs(log_dir, exist_ok=True)
			x_ori = torch.from_numpy(np.load(path)).cuda()
			x_adv_ori = torch.from_numpy(np.load(path_adv)).cuda()
					
			############ calculate the mean by the dimention of time 
			# 1) calculate bar_x_sigle_sample [100, 500, 3, 32, 32]
			# if dataset == 'cifar' or len(x.shape)!=4:
			# 	x = x[:t,:,:,:,:].mean(0)#.half() 
			...
			for i in range(100):
				...
				if i==0:
					print(f"The dimension of feature is {x.shape[0]}")
				dt_clean += dis_ref_clean2data(x, x).cpu()
				dt_adv += dis_ref_clean2data(x, x_adv).cpu()
				print(f"the first {i}:", plot_mi(dt_clean/(i+1), dt_adv/(i+1), log_dir, tile_name))

			num_bins = 50
			plt.hist(dt_clean/(i+1), num_bins, facecolor='red', density=True, alpha=0.9)
			plt.hist(dt_adv/(i+1), num_bins,  facecolor='blue', density=True,alpha=0.9)
			plt.title(f'{tile_name}')
			plt.savefig(f'{log_dir}/{tile_name}.jpg')
			plt.close()