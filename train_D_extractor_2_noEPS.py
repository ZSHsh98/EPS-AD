...
parser.add_argument('--sigma0', default=0.1, type=float, help="0.5 for imagenet")
parser.add_argument('--sigma', default=1, type=float, help="100 for imagenet")
...
batch_size =200 if dataset == 'imagenet' else 400
perb_image = True
...
if dataset == 'imagenet':
	path = f'/mnt/cephfs/ec/home/zhangshuhai/score_norm_{dataset}_noEPS/scores_cleansingle_vector_norm210000/'
	ref_data = DatasetNPY(path)
	ref_loader = DataLoader(ref_data, batch_size=batch_size, shuffle=True, num_workers=8)


	path_adv = f'/mnt/cephfs/ec/home/zhangshuhai/score_norm_{dataset}_noEPS/scores_adv_FGSM_L2_0.00392_5single_vector_norm210000/'
	adv_data1 = DatasetNPY(path_adv)
	adv_data_loader1 = DataLoader(adv_data1, batch_size=batch_size, shuffle=True, num_workers=8)

	path_adv2 = f'/mnt/cephfs/ec/home/zhangshuhai/score_norm_{dataset}_noEPS/scores_adv_FGSM_0.00392_5single_vector_norm210000/'
	adv_data2 = DatasetNPY(path_adv2)
	adv_data_loader2 = DataLoader(adv_data2, batch_size=batch_size, shuffle=True, num_workers=8)
else:
	path = f'/mnt/cephfs/ec/home/zhangshuhai/score_norm_{dataset}_noEPS/scores_cleansingle_vector_norm20perb_image10000/'
	ref_data = DatasetNPY(path)
	ref_loader = DataLoader(ref_data, batch_size=batch_size, shuffle=True, num_workers=8)


	path_adv = f'/mnt/cephfs/ec/home/zhangshuhai/score_norm_{dataset}_noEPS/scores_adv_FGSM_L2_0.00392_5single_vector_norm20perb_image10000/'
	adv_data1 = DatasetNPY(path_adv)
	adv_data_loader1 = DataLoader(adv_data1, batch_size=batch_size, shuffle=True, num_workers=8)

	path_adv2 = f'/mnt/cephfs/ec/home/zhangshuhai/score_norm_{dataset}_noEPS/scores_adv_FGSM_0.00392_5single_vector_norm20perb_image10000/'
	adv_data2 = DatasetNPY(path_adv2)
	adv_data_loader2 = DataLoader(adv_data2, batch_size=batch_size, shuffle=True, num_workers=8)
...
net = Discriminator(img_size=img_size, feature_dim=feature_dim) if dataset == 'imagenet' else Discriminator_cifar2(img_size=img_size, feature_dim=feature_dim)
net = net.cuda()
# model = get_image_classifier('imagenet-resnet18').cuda()
...
def test(epoch, diffusion_t, dataset):
	...
	attack_methods=['FGSM', 'FGSM_L2','BIM','TIM','BIM_L2' ] #'PGD','FGSM', 'BIM', 'MIM', 'TIM', 'DI_MIM','CW', 'PGD_L2', 'FGSM_L2', 'BIM_L2', 'MM_Attack', 'AA_Attack']
	# attack_method = 'PGD'
			...
			for attack_method in attack_methods:
				...
				for t in [tt]: #20, 50, 100, 150, 200]:
					...
					with torch.no_grad():
						...
						# with torch.no_grad():
						# 	feature_ref = model(ref_data)
						# 	feature_cln = model(x_cln)
						# 	feature_adv = model(x_adv)
...
# imagenet
# CUDA_VISIBLE_DEVICES=4 python train_D_extractor_2_noEPS.py --epochs 200 --lr 0.0002 --id 1 --sigma0 10000 --sigma 5  --epsilon 10 --feature_dim 300 --dataset imagenet
# cifar
# CUDA_VISIBLE_DEVICES=4 python train_D_extractor_2_noEPS.py --epochs 200 --lr 0.0002 --id 1 --sigma0 15 --sigma 100  --epsilon 10 --feature_dim 300 --dataset cifar