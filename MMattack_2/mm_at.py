...
from train_apgd import APGDAttack_topk

parser = argparse.ArgumentParser(description='PyTorch MM AT')
...
parser.add_argument('--out_dir', type=str, default='./MM_AT_resnet18', help='dir of output')
...
def train(model, train_loader, optimizer):
    ...
    apgd = APGDAttack_topk(model, n_restarts=1, n_iter=20, verbose=False,
            eps=args.epsilon, norm='Linf', eot_iter=1, rho=.75, seed=None, device='cuda', num_classes=args.num_classes)
    loss_sum = 0
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        model.eval()
        apgd.loss = 'mmk'
        x_adv_set = apgd.perturb(3, data, target)
        ...
        index = torch.argsort(logits-10000*target_var)[:,args.num_classes-3:]

        output1 = attack.mm_loss_train(model(x_adv_set[0]),target,index[:,0],num_classes=args.num_classes)
        output1 = output1.view(len(output1),-1)
        output2 = attack.mm_loss_train(model(x_adv_set[1]),target,index[:,1],num_classes=args.num_classes)
        output2 = output2.view(len(output2),-1)
        output3 = attack.mm_loss_train(model(x_adv_set[2]),target,index[:,2],num_classes=args.num_classes)
        output3 = output3.view(len(output3),-1) 

        loss_pipy = torch.cat((output1, output2),1)
        loss_pipy = torch.cat((loss_pipy,output3),1)

        index_choose = torch.argsort(loss_pipy)[:,-1]
        x_adv = torch.zeros(x_adv_set[0].size()).cuda()
        for i in range(len(index_choose)):
            x_adv[i,:,:,:] = x_adv_set[index_choose[i]][i]

        model.train()
        optimizer.zero_grad()
        output_adv = x_adv.clone().detach()
