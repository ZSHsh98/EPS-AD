	...
	for attack_method in attack_methods:
		...
		if not args.clean_score_flag:
			if not args.single_vector_norm_flag:
				np.save(f'{args.detection_datapath}/scores_adv_{attack_method}_{args.epsilon}_{args.num_steps}{isnorm}{value}{isperb_image}.npy', score_tensor.data.cpu().numpy())
			else:
				np.save(f'{args.detection_datapath}/scores_adv_{attack_method}_{args.epsilon}_{args.num_steps}single_vector_norm{value}{isperb_image}.npy', score_tensor.data.cpu().numpy())
		
		else:
			if not args.single_vector_norm_flag:
				np.save(f'{args.detection_datapath}/scores_clean{isnorm}{value}{isperb_image}.npy', score_tensor.data.cpu().numpy())
			else:
				np.save(f'{args.detection_datapath}/scores_clean{isnorm}single_vector_norm{value}{isperb_image}.npy', score_tensor.data.cpu().numpy())
...
def parse_args_and_config():
	...
	parser.add_argument('--detection_datapath', type=str, default='./score_diffusion_t_cifar')
...
# imagenet
# CUDA_VISIBLE_DEVICES=2 python eval_sde_adv.py --datapath '/mnt/cephfs/mixed/dataset/imagenet' --num_sub 500  --adv_batch_size 20 --detection_datapath './score_diffusion_t_imagenet'  --detection_flag --detection_ensattack_flag --detection_ensattack_norm_flag \
