...
def get_transform(dataset, transform_type, base_size=256):
    if dataset.lower() == "celebahq":
        ...
        # assert base_size == 128, base_size
...
def imagent_dataset_sub(root, transform=None, num_sub=-1, data_seed=0, train=False):
    val_data = torchvision.datasets.ImageFolder(os.path.join(root, 'val'), transform)
    # val_data = torchvision.datasets.ImageFolder(os.path.join(root), transform)
    if train:
         val_data = torchvision.datasets.ImageFolder(os.path.join(root, 'train'), transform)
    ...
    if num_sub > 0:
        if not train:
            ...
            partition_idx = np.linspace(0,len(val_data)-1,num_sub, dtype='int')
        val_data = Subset(val_data, partition_idx)

    return val_data

def cifar10_dataset_sub(root, transform=None, num_sub=-1, data_seed=0, train=False):
    val_data = torchvision.datasets.CIFAR10(root=root, transform=transform, download=True, train=train)
...
def cifar100_dataset_sub(root, transform=None, num_sub=-1, data_seed=0, train=False):
    val_data = torchvision.datasets.CIFAR100(root=root, transform=transform, download=True, train=train)
