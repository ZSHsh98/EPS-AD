...
parser.add_argument('--output_dir', default='./data/adv/mi-fgsm/inceptionv3', type=str, help='directory of crafted adversarial examples')
...
def mi_fgsm(x, y, model, use_gpu=True):
    '''
...
def main():
    ...
    for i, (x, y) in enumerate(clean_loader):
        if args.use_gpu:
            x = x.cuda()
            y = y.cuda()
        adv_x, corr_1, corr_5 = mi_fgsm(x, y, model, use_gpu=args.use_gpu)
        correct_1 += corr_1
        correct_5 += corr_5
