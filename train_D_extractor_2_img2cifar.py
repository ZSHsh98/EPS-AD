...
if 1:
	path = f'/mnt/cephfs/ec/home/zhangshuhai/score_diffusion_t_imagenet2cifar/scores_cleansingle_vector_norm{t}{isperb_image}10000/'
	ref_data = DatasetNPY(path)
	ref_loader = DataLoader(ref_data, batch_size=batch_size, shuffle=True, num_workers=8)


	path_adv = f'/mnt/cephfs/ec/home/zhangshuhai/score_diffusion_t_imagenet2cifar/scores_adv_FGSM_L2_0.00392_5single_vector_norm{t}{isperb_image}10000/'
	adv_data1 = DatasetNPY(path_adv)
	adv_data_loader1 = DataLoader(adv_data1, batch_size=batch_size, shuffle=True, num_workers=8)

	path_adv2 = f'/mnt/cephfs/ec/home/zhangshuhai/score_diffusion_t_imagenet2cifar/scores_adv_FGSM_0.00392_5single_vector_norm{t}{isperb_image}10000/'
...
def test(epoch, diffusion_t, dataset):
	...
	if not args.test_flag:
		model_path = f'./net_D/img2cifar/{id}' 
		state = {
				'net': net.state_dict(),
...
# imagenet
# CUDA_VISIBLE_DEVICES=4 python train_D_extractor_2_img2cifar.py --epochs 200 --lr 0.002 --id 6 --sigma0 0.5 --sigma 100  --epsilon 10 --feature_dim 300 --dataset imagenet